...
from app import app
from .models import UsersTable, BooksTable, BookHistory, ActiveTokens, RevokedTokens
...
def post_book():
    ...
    if request.method == "POST":
        ...
        return Response(json.dumps({'message': 'You have successfully added the book {} by {} the year {}'.format(title, author, year)}),\
    ...
    try:
        ...
        if not book_to_update:
            return Response(json.dumps({'message': 'This book does not exist in the library.'}),\
             status=200, content_type = 'apllication/json')
            ...
            return Response(json.dumps({'message': 'You have deleted this book {}'.format(book_to_update.title)}),
             status=200,content_type='application/json')
    except ValueError:
...
def borrow_book(book_id):
    ...
    if usermail == None or usermail == "" or usermail.strip() =="":
        ...
        if logged_in_user != usermail or RevokedTokens.is_jti_blacklisted(jti):
            ...
            book_details = BooksTable.get_book_by_id(bookID)
            if not book_details:
               return Response(json.dumps({'message': 'The library has no book with that ID'}))
                ...
                if request.method == "POST":
                    ...
                    if book_details.is_not_borrowed == "True":
                        return Response(json.dumps({'Message': 'This book has not been borrowed {}'.format(book_to_return)}),\
                         status = 200, content_type='application/json')
...
def borrow_history():
    ...
    current_user = UsersTable.retrieve_user_by_email(usermail)
    ...
    if returned and returned == "false":
        ...
        if not books_not_returned:
            ...
            return jsonify(current_user.get_unreturned()), 200
            
    else:
        books_borrowed = current_user.get_user_history()
        if not books_borrowed:
            return Response(json.dumps({"Message": "You do not have a book that is not returned."}), status = 404,\
             content_type='application/json')
        else:
            return jsonify(current_user.get_user_history()), 200
